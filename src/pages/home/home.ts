...
export class HomePage implements OnInit{
  ...
  listaLivros(){
        ...
        this.loader.present();
        this.livrosService.loadLivros(result.id_user).subscribe(data => {
            if(data.status == 0){
                this.books = data.livros;
                this.loader.dismiss();
            } else {
                this.loader.dismiss();
                this.presentAlert("Ops...","Não existem livros cadastrados ainda.");
            }
        }, error =>{
            this.loader.dismiss();
  ...
  checkNetwork(){
        ...
        let onOffline = () => {
            if(this.showMessage == false){
                this.showMessage = true;
                this.presentAlert("Ops...","Voce esta offline");
            }
        };
  ...
  searchLivro(palavra){
        this.livrosService.searchLivros(palavra).subscribe(dataSearch => {
          if(dataSearch.status == 0){
              this.resultBusca = [];
              this.resultBusca = dataSearch.lista;
              //for(var key in data){
            ...
            //busca veio vazia, nao tem nada encontrado
          }
        }, error =>{
              this.presentAlert("Ops...","Não foi possível se comunicar com o servidor. Tente mais tarde.");
        });
